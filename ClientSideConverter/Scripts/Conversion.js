$(document).ready(function () {
    ...
    function convertUnits(conversionFactors) {
        var inputValue = parseFloat($('#UnitToConvert').val());
        if (isNaN(inputValue)) {
            Swal.fire({
                title: 'Not A Number',
        ...
        var fromUnit = $('#FromSelect').val();
        var toUnit = $('#ToSelect').val();

        var valueInBaseUnit = inputValue * conversionFactors[fromUnit];

        var resultValue = valueInBaseUnit / conversionFactors[toUnit];

        $('#UserInputValue').text(inputValue);
        $('#ConvertFromUnitDisplay').text(fromUnit);
        $('#ResultValue').text(resultValue.toFixed(4));
        $('#ConvertToUnitDisplay').text(toUnit);
    }

    $('#UnitToConvert').on('keypress', function (e) {
        if (e.which === 13) {
            $('#ConvertButton').click();
        }
    });
    ...
    $('#ConvertButton').click(function () {
        var conversionType = $(this).data('conversion');

        var conversionFactors = conversionFactorsList[conversionType];
        ...
        if (conversionFactors) {
            ...
            Swal.fire({
                title: 'Not found',
                text: 'Conversion type not found',
                icon: 'error',
                confirmButtonText: 'Ok'
            })
        }
